...
variable "db_subnet_ingress_rules" {
  description = "The db security list rules. Defaults contain mysql"
...
7. Import child modules into the main module using `source` attribute from various sources: <br>
...
resource "aws_eip" "main_reserved_lb_ip" {
  count = 1
  tags = merge({ "Name" = "main_reserved_ip_${count.index}"}, local.tags)
}
```
...
11. Use `for_each` and `each` iteration primitives (`.key`, `value.*`) for granular data structure looping over sets.
...
variable "db_system_storage_iops" {
  description = "(Applicable when system_type=OCI_OPTIMIZED_STORAGE) DbSystem Performance Unit"
  default     = "300000"
  type        = string
  validation {
